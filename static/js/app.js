...
window.RENDER_BACKEND_URL = window.RENDER_BACKEND_URL || "https://software-2-p1lq.onrender.com";
...
async function loadAppBrandingSettings() {
  try {
    ...
    if (sidebarLogo && settings.software_logo) {
      sidebarLogo.src = settings.software_logo;
    }
    ...
    if (loginLogo && settings.software_logo) {
      loginLogo.src = settings.software_logo;
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  loadKpis();
  loadDashboardTasks();
  selectLoginRole('DOCTOR', document.getElementById('loginRoleDoctor'));
  loadDoctorsList();
  loadQueueData();
  loadBedsData();
  ...
  setTimeout(() => {
    showToast('Arfa Nova Technology', 'System online — All modules loaded ✓', 'success', 3500);
  }, 800);
});
...
function playNotificationSound(tone = 'chime', volume = 80) {
  try {
    ...
    const vol = Math.max(0, Math.min(100, parseInt(volume) || 80)) / 100;
